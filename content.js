...
$(document).on('keydown', function(e) {
    //if ((e.ctrlKey) && e.shiftKey && e.which === 69) { //used to be Ctrl+Shift+E, but this is way too close to Ctrl+Shift+W which closes the browser!
    if (e.ctrlKey && e.shiftKey && e.which === 190) { //190= new hotkey will be Ctrl+Shift+. (period)
        enable(!enabled);
    }else if (e.which === 27 && enabled) { //if enabled and the Esc is pressed, cancels out of the current erase state
		enable(false);
	}else if (e.ctrlKey && e.which === 90) { //Ctrl+Z fires the undo
		//enable(false);
		//alert('blah');
		chrome.extension.sendMessage({command:'undo_eraser', data: ''});
...
function enable(state) {
    if (state) {
        ...
        doc.on('mousedown mouseup', onMouseNothing);
        ...
        doc.off('mousedown mouseup', onMouseNothing);
